...
async function openDocPiP(){
  ...
  // 1) Olay ile güncelle (varsa)
  const unCounter = sub('counter', (val) => { v.textContent = String(val); });

  // 2) GARANTİ SENKRON: her saniye ana pencerenin snapshot'ını uygula
  const syncTimer = setInterval(() => {
    if (pip.closed) { clearInterval(syncTimer); return; }
    applyDash(window.__KZS_LAST_DASH__); // dashboard.js her saniye güncelliyor
  }, 1000);
